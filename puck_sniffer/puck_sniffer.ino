...
static inline uint16_t rnext(uint16_t i){ return (uint16_t)((i + 1) % RINGN); }
...
static uint8_t  g_forceCh = 0;            // !=0 -> user pinned a channel (skip auto-acquire retune)
static bool     g_pinSession = false;     // user pinned a FULL session (cmd 05): never auto-retune addr/ch
static bool     g_survey = false;         // cmd 06: stay on ibex/ch2 and stream EVERY E1 beacon, never auto-lock
...
static uint8_t  g_lastMatch = 0;          // RXMATCH pipe of the most recent frame (diagnostic)
...
static void tuneDiscovery(){ g_state=0; g_curCh=2; rfConfig(2); rfSetAddr(PAIR_BASE, 0x10); rxArmInit(); }
...
static void tuneSession(){ g_state=1; rfConfig(g_curCh); rfSetAddrMulti(g_sBase, g_sPrefix); rxArmInit(); }
...
static void rxArmInit(){
  ...
  NRF_RADIO->EVENTS_END = 0; NRF_RADIO->EVENTS_ADDRESS = 0;
  NRF_RADIO->TASKS_RXEN = 1;
}

static void ringPush(uint32_t t, uint8_t ch, bool crcok, uint8_t rssi, uint8_t match, const uint8_t* b, uint8_t n){
  uint16_t h = g_head, nx = rnext(h);
  if (nx == g_tail) g_tail = rnext(g_tail);   // full -> drop oldest
  Cap &c = g_ring[h];
  c.t = t; c.ch = ch; c.flags = crcok ? 1 : 0; c.rssi = rssi; c.match = match; c.n = n;
  memcpy(c.b, b, n);
  g_head = nx;
}
...
static void rxPump(){
  for (int guard = 0; guard < 8 && NRF_RADIO->EVENTS_END; guard++){
    ...
    if (crcok && len >= 1 && (rfrx[2] & 0xF0) == 0xF0) g_lastFrx = millis();   // controller reply = ACTIVE session here -> stop hunting, park
    ...
    if (crcok && len >= 18 && rfrx[2] == 0xE1){
      g_advCh = rfrx[11];
      g_advBase[0]=rfrx[15]; g_advBase[1]=rfrx[16]; g_advBase[2]=rfrx[17]; g_advBase[3]=rfrx[18];
      g_advPfx = rfrx[19];
      if (g_pinSession || g_survey) continue;                // pinned / survey -> never auto-retune (just record adv)
      ...
      if (g_haveTarget && memcmp(rfrx+7, g_targetIbex, 4) != 0) continue;
      // ACQUIRE: lock onto the advertised session. CAPTURE: a different advertised channel = QoS hop -> follow.
      if (g_advCh >= 1 && g_advCh <= 80){
        if (g_state == 0){
          memcpy(g_sBase, g_advBase, 4); g_sPrefix = g_advPfx; g_primaryCh = g_advCh;
          g_curCh = g_forceCh ? g_forceCh : g_advCh; g_huntIdx = 0; g_lastFrx = millis();
          tuneSession(); return;
        } else if (!g_forceCh && g_advCh != g_curCh){
          g_curCh = g_advCh; tuneSession(); return;
        }
      }
    }
  }
...
static void emitStatus(){
  static unsigned long last=0; if (millis()-last < 250) return; last=millis();
  static uint16_t rx=0; rx++;
  uint8_t f[19]={0xC1,0xDE,g_state,g_curCh,g_sBase[0],g_sBase[1],g_sBase[2],g_sBase[3],g_sPrefix,
                 (uint8_t)(g_streaming?1:0),(uint8_t)rx,(uint8_t)(rx>>8),
                 g_advCh,g_advBase[0],g_advBase[1],g_advBase[2],g_advBase[3],g_advPfx,g_lastMatch};
  usb_web.write(f, sizeof f); usb_web.flush();
}
static void drainToHost(){
  if (!usb_web.connected()) return;
  uint16_t budget = 64;   // bound per-call so RX stays responsive; the ring covers bursts
  while (budget-- && g_tail != g_head){
    if (g_streaming) emitPacket(g_ring[g_tail]);
    g_tail = rnext(g_tail);
  }
  usb_web.flush();
  emitStatus();
...
static void handleCmd(){
  while (usb_web.available()){
    ...
    if (c == 0x01){ g_streaming = true; g_head = g_tail = 0; }   // START (flush stale ring)
    else if (c == 0x02){ g_streaming = false; }                 // STOP
    else if (c == 0x03){ g_forceCh = 0; g_pinSession = false; g_survey = false; tuneDiscovery(); }   // re-acquire from ibex/ch2
    else if (c == 0x06){ g_survey = !g_survey; g_forceCh = 0; g_pinSession = false; tuneDiscovery(); }   // survey: list all E1 beacons on ch2, no lock
    else if (c == 0x07){                                       // target ibex_uuid: lock only the matching slot's E1
      int u0=usb_web.read(),u1=usb_web.read(),u2=usb_web.read(),u3=usb_web.read();
      if (u0>=0&&u1>=0&&u2>=0&&u3>=0){
        g_targetIbex[0]=u0; g_targetIbex[1]=u1; g_targetIbex[2]=u2; g_targetIbex[3]=u3;
        g_haveTarget=true; g_survey=false; g_forceCh=0; g_pinSession=false; tuneDiscovery();
      }
    }
    else if (c == 0x04){ int ch = usb_web.read(); if (ch>=1 && ch<=80){ g_forceCh=(uint8_t)ch; g_curCh=(uint8_t)ch; tuneSession(); } }   // pin channel, keep addr
    else if (c == 0x05){                                        // pin a FULL session: b0 b1 b2 b3 pfx ch
      int b0=usb_web.read(),b1=usb_web.read(),b2=usb_web.read(),b3=usb_web.read(),pf=usb_web.read(),ch=usb_web.read();
      if (b0>=0&&b1>=0&&b2>=0&&b3>=0&&pf>=0&&ch>=1&&ch<=80){
        g_sBase[0]=b0; g_sBase[1]=b1; g_sBase[2]=b2; g_sBase[3]=b3; g_sPrefix=(uint8_t)pf;
        g_curCh=(uint8_t)ch; g_forceCh=(uint8_t)ch; g_pinSession=true; tuneSession();
      }
    }
  }
}
...
void setup(){
  ...
  USBDevice.setID(0x28DE, 0x534E);                 // 'SN' -- distinct PID so the host doesn't confuse it with a puck
  USBDevice.setManufacturerDescriptor("Valve Software");
  USBDevice.setProductDescriptor("OpenPuck Sniffer");
  usb_web.begin();
  Serial.begin(115200);
  for (int i=0;i<200 && !USBDevice.mounted(); i++) delay(10);
...
void loop(){
  ...
  if (g_state == 1 && !g_forceCh && !g_pinSession
      && millis() - g_lastFrx > HUNT_NOFRX_MS && millis() - g_huntMs > HUNT_DWELL_MS){
    g_huntMs = millis();
    uint8_t ch = (g_huntIdx == 0) ? g_primaryCh : HUNT_CH[(g_huntIdx - 1) % (sizeof HUNT_CH)];
    g_huntIdx++;
    if (g_huntIdx > 1 + 2 * (uint8_t)sizeof HUNT_CH){ g_huntIdx = 0; tuneDiscovery(); g_lastFrx = millis(); g_huntMs = millis(); return; }
    g_curCh = ch; rfConfig(g_curCh); rfSetAddrMulti(g_sBase, g_sPrefix); rxArmInit();
  }
}
